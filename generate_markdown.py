...
md.append("")
md.append("A complete reference database of **89+ canonical Aniimo species** across **Basic Forms**, **Weather Forms**, and **Prismana Forms**, detailing their specific **Homeland Abilities (Levels 1–5)**, workplace perks, and best roles.")
md.append("")
md.append("---")
md.append("")
...
table_headers = "| No. | Aniimo | Stage | Type | Role | Basic Abilities | Weather Forms | Prismana Abilities |"
table_divider = "| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |"
...
for item in data:
    ...
    row = f"| **#{item['id']}** | [{item['name']}](#{item['name'].lower()}) | `{item['stage']}` | {item['element']} | {item.get('role', 'DPS')} | {b_abils} | {w_str} | {p_abils} |"
    md.append(row)
...
for item in data:
    ...
    md.append(f"**Stage:** `{stage}` | **Element:** `{item['element']}` | **Combat Role:** `{item.get('role', 'DPS')}` | **Evolution:** `{evo}`")
